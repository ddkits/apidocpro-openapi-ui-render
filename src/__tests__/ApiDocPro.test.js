...
const THEME = {
  TEMPLATES: {
    ...
    infoCollapsible:
      '<details  id="%KEY%" class="p-2" ><summary>%KEY%</summary>%CHILDREN%</details>',
    infoCollapsibleOpen:
      '<details id="%KEY%" class=" p-2" open><summary class="apidocpro__key apidocpro__method--%KEY%"><b>%KEY%</b><span class="apidocpro__value apidocpro__value--type-%TYPE%">%VALUE%</span></summary>%CHILDREN%</details>'
  },
  TEMPLATESASYNC: {
...
test('renders RealLexi LLC', () => {
  render(<ApiDocPro data={spec} theme={THEME} />);
  const linkElement = screen.getByText(/RealLexi LLC/i);
  expect(linkElement).toBeInTheDocument();
});

test('renders RealLexi LLC no right region', () => {
  render(<ApiDocPro data={spec} theme={THEME} rightregion={false} />);
  const linkElement = screen.getByText(/RealLexi LLC/i);
  expect(linkElement).toBeInTheDocument();
});
